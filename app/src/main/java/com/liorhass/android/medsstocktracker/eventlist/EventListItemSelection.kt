...
// TODO: Unite this with the equivalent classes of medicineList

// Provide the SelectionTracker item's position given a key, and a key given position
class LoggedEventListItemKeyProvider(private val recyclerView: RecyclerView) :
...
class LoggedEventListItemDetailsLookup(private val recyclerView: RecyclerView) : ItemDetailsLookup<Long>() {
    override fun getItemDetails(event: MotionEvent): ItemDetails<Long>? {
        val view = recyclerView.findChildViewUnder(event.x, event.y)
        if (view != null) {
            val viewHolder = recyclerView.getChildViewHolder(view)
            if (viewHolder is EventListAdapter.ViewHolder) {
